...
final class SQLiteBooksDatabase extends BooksDatabase {
    ...
    protected List<Label> listLabels(long bookId) {
        final Cursor cursor = myDatabase.rawQuery("SELECT Labels.name,BookLabel.uid FROM Labels" +
                                                  " INNER JOIN BookLabel ON BookLabel.label_id=Labels.label_id" +
                                                  " WHERE BookLabel.book_id=?", new String[]{String.valueOf(bookId)});
    ...
    protected List<String> listLabels() {
        final Cursor cursor = myDatabase.rawQuery("SELECT DISTINCT(Labels.name) FROM Labels" +
                                                  " INNER JOIN BookLabel ON BookLabel.label_id=Labels.label_id" +
                                                  " INNER JOIN Books ON BookLabel.book_id=Books.book_id" +
                                                  " WHERE Books.`exists`=1", null);
    ...
    protected boolean hasVisibleBookmark(long bookId) {
        final Cursor cursor = myDatabase.rawQuery("SELECT bookmark_id FROM Bookmarks WHERE book_id = " + bookId +
    ...
    private void createTables() {
        myDatabase.execSQL("CREATE TABLE IF NOT EXISTS Books(" +
                           "book_id INTEGER PRIMARY KEY," +
                           "encoding TEXT," +
                           "language TEXT," +
                           "title TEXT NOT NULL," +
                           "file_name TEXT UNIQUE NOT NULL)");
        myDatabase.execSQL("CREATE TABLE IF NOT EXISTS Authors(" +
        ...
        myDatabase.execSQL("CREATE TABLE IF NOT EXISTS Series(" +
                           "series_id INTEGER PRIMARY KEY," +
                           "name TEXT UNIQUE NOT NULL)");
        myDatabase.execSQL("CREATE TABLE IF NOT EXISTS BookSeries(" +
                           "series_id INTEGER NOT NULL REFERENCES Series(series_id)," +
                           "book_id INTEGER NOT NULL UNIQUE REFERENCES Books(book_id)," +
                           "book_index INTEGER)");
        myDatabase.execSQL("CREATE TABLE IF NOT EXISTS Tags(" +
                           "tag_id INTEGER PRIMARY KEY," +
                           "name TEXT NOT NULL," +
                           "parent INTEGER REFERENCES Tags(tag_id)," +
                           "CONSTRAINT Tags_Unique UNIQUE (name, parent))");
        myDatabase.execSQL("CREATE TABLE IF NOT EXISTS BookTag(" +
    ...
    private void updateTables3() {
        myDatabase.execSQL("CREATE TABLE IF NOT EXISTS Files(" +
    ...
    private void updateTables4() {
        ...
        myDatabase.execSQL("CREATE TABLE IF NOT EXISTS RecentBooks(" +
                           "book_index INTEGER PRIMARY KEY," +
                           "book_id INTEGER REFERENCES Books(book_id))");
    }

    private void updateTables5() {
        myDatabase.execSQL("CREATE TABLE IF NOT EXISTS Bookmarks(" +
        ...
        myDatabase.execSQL("CREATE TABLE IF NOT EXISTS BookState(" +
    ...
    private void updateTables8() {
        myDatabase.execSQL("CREATE TABLE IF NOT EXISTS BookList ( " + "book_id INTEGER UNIQUE NOT NULL REFERENCES Books (book_id))");
    }
    ...
    private void updateTables10() {
        myDatabase.execSQL("CREATE TABLE IF NOT EXISTS Favorites(" + "book_id INTEGER UNIQUE NOT NULL REFERENCES Books(book_id))");
    }
    ...
    private void updateTables13() {
        myDatabase.execSQL("ALTER TABLE Bookmarks ADD COLUMN visible INTEGER DEFAULT 1");
    }
    ...
    private void updateTables16() {
        myDatabase.execSQL("ALTER TABLE Books ADD COLUMN `exists` INTEGER DEFAULT 1");
    }
    ...
    private void updateTables20() {
        myDatabase.execSQL("CREATE TABLE IF NOT EXISTS Labels(" +
                           "label_id INTEGER PRIMARY KEY," +
                           "name TEXT NOT NULL UNIQUE)");
        myDatabase.execSQL("CREATE TABLE IF NOT EXISTS BookLabel(" +
    ...
    private void updateTables21() {
        ...
        myDatabase.execSQL("CREATE TABLE IF NOT EXISTS BookUid(" +
                           "book_id INTEGER NOT NULL UNIQUE REFERENCES Books(book_id)," +
                           "type TEXT NOT NULL," +
                           "uid TEXT NOT NULL," +
    ...
    private void updateTables23() {
        myDatabase.execSQL("CREATE TABLE IF NOT EXISTS HighlightingStyle(" +
    ...
    private void updateTables25() {
        myDatabase.execSQL("CREATE TABLE IF NOT EXISTS BookReadingProgress(" +
    ...
    private void updateTables26() {
        myDatabase.execSQL("CREATE TABLE IF NOT EXISTS BookHash(" +
                           "book_id INTEGER PRIMARY KEY REFERENCES Books(book_id)," +
    ...
    private void updateTables29() {
        ...
        myDatabase.execSQL("CREATE TABLE IF NOT EXISTS BookHistory(" +
                           "book_id INTEGER REFERENCES Books(book_id)," +
                           "timestamp INTEGER NOT NULL," +
